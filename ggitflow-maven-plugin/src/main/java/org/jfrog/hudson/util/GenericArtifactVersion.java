...
/**
 * Represents a generic standard artifact version. This class can be used either
 * for parsing a version string and/or for building a version string
 * programatically. Consistency is guaranteed at all times so that no invalid
 * version is formed. Also, new
 * {@link GenericArtifactVersion}(versionString).toString().equals(versionString)
 * is guaranteed to be always true if 'versionString' is in the right format.
 *
 ...
 * <ul>
 * where:
 * <li>primary_numbers: <p1>[.<p2>[...<pn>]...] are non-negative integers
 * separated by '.'. Examples: 1; 1.2; 1.2.3; etc.</li>
 * <li>annotation: Sequence of letters that usually describe the release type.
 * Examples: alpha, beta, RC, etc.</li>
 * <li>annotation_revision: A non-negative integer associated to the
 * annotation.</li>
 * <li>build_specifier: SNAPSHOT|YYYYMMdd.hhmmss-n[n[...[n]...]] Examples:
 * SNAPSHOT, 20120110.152615-1, 20120110.152615-543</li>
 * </ul>
 *
 * Note that we used generic terms as components. Even though "major", "minor",
 * and "incremental" are terms used for the first three primary components, we
 * want to abstract that to be as generic as possible. Other implementations can
 * reuse this class and rename components as desired.
 *
 * Notes: The version format is based on Maven's
 * org.apache.maven.shared.release.versions.DefaultVersionInfo. As of today,
 * that class can be found in the following artifact:
 * org.apache.maven.release:maven-release-manager:2.2
 * <p>
 * The reason why I decided to reimplement this is becase of the unwanted
 * dependencies added with that artifact that clash with some Jenkins'
 * dependencies. Besides that, the implementation is not very flexible, it does
 * not provide means to retrieve the component separators and it cannot be
 * extended either. Moreover, if we depended on that class, our code can easily
 * break if we upgrade that artifact and they update their version scheme. What
 * we do when releasing is tied to our interpretation of version schemes.
 *
 *
...
public class GenericArtifactVersion {
    ...
    public static final String VERSION_REGEX = "^" + VERSION_PRIMARY_NUMBERS_REGEX + "(" + VERSION_COMPONENT_SEPARATOR_REGEX
     ...
     *
     * @param primaryNumbers A string representing a sequence of primary numbers
     * separated by dot ('.'). Examples: "1", "1.2.3". Must be in the correct
     * format.
     * @return An array where each element is an integer. Index 0 contains the
     * most significant primary number.
     */
    ...
    /**
     * Parses a version component separator string and returns it as a
     * character.
     *
     * @param separator The separator string to parse.
     * @return The first character of the string, or null if that string is
     * empty.
     */
     ...
     *
     * @param annotationRev The string to parse.
     * @return An integer representing the version annotation revision, or null
     ...
     * <p>
     * If the annotation revision is defined, it's incremented. Otherwise, the
     * last primary number is incremented.
     * <p>
     * Examples:
     * <ul>
    ...
    /**
     * Increments the least significant primary number by 1, resetting the
     * annotation revision if it is defined.
     * <p>
    ...
    /**
     * Increments a primary number by 1, resetting the remaining less
     * significant components.
     * <p>
     * Examples (assuming a base annotation revision of 1):
     * <ul>
     * <li>"1.2.3.4-alpha-5" is upgraded to "2.0.0.0-alpha-1" with an index of
     * 0</li>
     * <li>"1.2.3.4-alpha-5" is upgraded to "1.2.3.0-alpha-1" with an index of
     * 3</li>
     * <li>"1.2-alpha" is upgraded to "1.3-alpha" with an index of 1</li>
     * <li>"1-SNAPSHOT" is upgraded to "2-SNAPSHOT" with an index of 0</li>
     * </ul>
     *
     * @param primaryNumberIndex The index identifying the primary number to
     * increase.
     * @return The instance, for method chaining.
     * @throws IllegalArgumentException if the index is negative.
     * @throws IllegalStateException if this version doesn't have a primary
     ...
     *
     * @return The instance, for method chaining.
     * @throws IllegalStateException if this version doesn't have an annotation
     * revision defined.
     */
    ...
    /**
     * Resets (sets to 0) all consecutive primary numbers starting at a specific
     * index.
     *
     * @param fromIndex The index pointing to the first primary number that will
     * be reset.
     */
    ...
    /**
     * Resets the annotation revision (if it is defined). It is set to
     * {@link #DEFAULT_BASE_ANNOTATION_REVISION}.
     */
     ...
     * <p>
     * The string representation is the concatenation of all numbers separated
     ...
     *
     * @param index The index pointing to the primary number. Must be
     * non-negative.
     * @return The primary number, or null if it's not defined.
     * @throws IllegalArgumentException if the index is provided is negative.
     */
     ...
     *
     * @param index The index pointing to the primary number to define. Must be
     * non-negative.
     * @param value The number to define as the primary component. Must be
     * non-negative.
     * @return The instance, for method chaining.
     * @throws IllegalArgumentException if the index or value is negative, or if
     * a primary number does not exist at that index.
     */
     ...
     *
     * @param annotationSeparator A valid separator character (read this class'
     * documentation for more information about the valid format). null can be
     * used to indicate that there's no separator.
     * @return The instance, for method chaining.
     * @throws IllegalArgumentException if the character provided is invalid.
     */
     ...
     *
     * @param annotation A valid annotation string (read this class'
     * documentation for more information about the valid format). null can be
     * used to indicate that there's no annotation.
     * @return The instance, for method chaining.
    ...
    /**
     * Defines the separator character for the version annotation revision
     * component.
     *
     * @param annotationRevisionSeparator A valid separator character (read this
     * class' documentation for more information about the valid format). null
     ...
     *
     * @param annotationRevision The revision number. Must be non-negative. null
     * can be used to indicate that there's no annotation revision number.
     * @return The instance, for method chaining.
     * @throws IllegalArgumentException if 'annotationRevision' is negative.
     */
    ...
    /**
     * Defines the separator character for the version build specifier
     * component.
     *
     * @param buildSpecifierSeparator A valid separator character (read this
     * class' documentation for more information about the valid format). null
     * can be used to indicate that there's no separator.
     * @return The instance, for method chaining.
     * @throws IllegalArgumentException if the character provided is invalid.
     ...
     *
     * @param buildSpecifier A valid build specifier (read this class'
     * documentation for more information about the valid format). null can be
     * used to indicate that there's no build specifier component.
     * @return The instance, for method chaining.
     * @throws IllegalArgumentException if the build specifier is invalid.
     */
    ...
    /**
     * Checks that a character is a valid version component separator, only if
     * it's not null.
     *
     * @param separator The separator character that we want to check.
     * @throws IllegalArgumentException if the character is not a valid version
     * component separator.
     */
     ...
     * <p>
     * The following condition is always true: new
     * {@link GenericArtifactVersion} (versionString).equals(versionString)
     * <p>
     * The result is the concatenation of all the version components, separated
     * with their respective separator characters:
     * <ul>
    ...
    /**
     * Creates a string concatenating the primary numbers separated by the dot
     * character ('.').
     * <p>
     * The result is appended to the provided string builder and returned.
     *
     * @param result The string builder where to append the string.
     ...
     * @return
     */
    private StringBuilder createAnnotationString(final StringBuilder result) {
        if (null != this.annotation) {
            result.append(this.annotationSeparator).append(this.annotation);
        }
        if (null != this.annotationRevision) {
            result.append(this.annotationRevisionSeparator).append(this.annotationRevision);
        }
        return result;
    }
